export default {
  ...
  const FETCH_STRATEGIES = {
    direct_regex: async (url) => {
        ...
        const trs = [...text.matchAll(trRegex)];

        if (!trs.length) return [];

        // dcMap: 数据中心三字母 => 国家代码
        const dcMap = {
            "ATL":"US","PEK":"CN","LAX":"US","DXB":"AE","HND":"JP","ORD":"US","LHR":"GB",
            "PVG":"CN","CDG":"FR","DFW":"US","AMS":"NL","FRA":"DE","IST":"TR","CAN":"CN",
            "JFK":"US","SIN":"SG","ICN":"KR","DEN":"US","BKK":"TH","SFO":"US","KUL":"MY",
            "MAD":"ES","LAS":"US","FCO":"IT","YYZ":"CA","MIA":"US","BCN":"ES","MUC":"DE",
            "SYD":"AU","SVO":"RU","SHA":"CN","PHX":"US","IAH":"US","MEL":"AU","CLT":"US",
            "EWR":"US","MSP":"US","DTW":"US","SEA":"US","BOM":"IN","DEL":"IN","TPE":"TW",
            "MAN":"GB","FUK":"JP","SJC":"US","ZRH":"CH","IAD":"US","LIS":"PT","CPH":"DK",
            "OSL":"NO","HEL":"FI","WAW":"PL","VIE":"AT","ARN":"SE","DUB":"IE","EDI":"GB",
            "GIG":"BR","GRU":"BR","PTY":"PA","JNB":"ZA","CPT":"ZA","CAI":"EG","DOH":"QA",
            "KWI":"KW","RUH":"SA","AUH":"AE","MCT":"OM","AMM":"JO","TLV":"IL","BEG":"RS",
            "SOF":"BG","OTP":"RO","SKG":"GR","ATH":"GR","TUN":"TN","CMN":"MA","RAK":"MA",
            "ADD":"ET","NBO":"KE","DAR":"TZ","LUN":"ZM","LAD":"AO","LOS":"NG","ABV":"NG",
            "ACC":"GH","KTM":"NP","DAC":"BD","HKG":"HK","MFM":"MO","BNE":"AU","PER":"AU",
            "CNS":"AU","ADL":"AU","OOL":"AU","AKL":"NZ","CHC":"NZ","NRT":"JP","KIX":"JP",
            "NGO":"JP","CTS":"JP","OKA":"JP","OSA":"JP","HKT":"TH","CNX":"TH","DMK":"TH",
            "SGN":"VN","HAN":"VN","PEN":"MY","CGK":"ID","DPS":"ID","SUB":"ID","MNL":"PH",
            "CEB":"PH","KHI":"PK","LHE":"PK","ISB":"PK","MEX":"MX","CUN":"MX","GDL":"MX",
            "EZE":"AR","SCL":"CL","LIM":"PE","UIO":"EC","BOG":"CO","CCS":"VE","GYE":"EC",
            "SAL":"SV","TIA":"AL","TBS":"GE","KBP":"UA","IEV":"UA","MSQ":"BY","VKO":"RU",
            "DME":"RU","LED":"RU","SVX":"RU","KZN":"RU","OMS":"RU","ROV":"RU","KUF":"RU",
            "GYD":"AZ","BAK":"AZ","BUD":"HU","PRG":"CZ","VNO":"LT","RIX":"LV","TLL":"EE",
            "ZAG":"HR","LJU":"SI","SKP":"MK","MLA":"MT","NCE":"FR","LYS":"FR","MRS":"FR",
            "TLS":"FR","BOD":"FR","NAN":"FJ","PNQ":"IN","BLR":"IN","HYD":"IN","MAA":"IN",
            "CCU":"IN","AMD":"IN","TRV":"IN","COK":"IN"
        };
        ...
        for (const tr of trs) {
            const tds = tr[1].match(/<td[^>]*>(.*?)<\/td>/g);
            if (!tds || tds.length < 6) continue;

            // 获取线路名称
            const lineName = tds[0].replace(/<[^>]+>/g, '').trim(); // 移动/联通/电信

            // 获取 IP
            const ip = tds[1].replace(/<[^>]+>/g, '').trim();

            // 获取数据中心前3个字母
            const dcFull = tds[5].replace(/<[^>]+>/g, '').trim();
            const dc = dcFull.substring(0,3);
            const country = dcMap[dc] || 'CN';

            for (const port of ports) {
                nodes.push(`${ip}:${port}#${lineName}-${country}-${port}-${protocol}-${security}`);
            }
        }
